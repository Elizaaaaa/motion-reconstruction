...
video_dir = osp.join(curr_path, '..', 'data')
output_dir = osp.join(curr_path, '..', 'refine_output')
...
flags.DEFINE_string('video_dir', video_dir, 'path to original videos')

flags.DEFINE_string('out_dir', output_dir, 'path to the output')
...
flags.DEFINE_float('joint_smooth_weight', 40, 'weight on joint smoothness')
flags.DEFINE_float('camera_smooth_weight', 1., 'weight on camera smoothness')
flags.DEFINE_float('init_pose_loss_weight', 120., 'weight on how much to stick to initial pose')
